...
// Register all the events using the path given
   ...
   registerEvents(filePath = this.eventsPath, client = this) {
    ...
    if(!fs.existsSync(filePath)) throw new Error(`Path ${filePath} for events was not found`)// path does not exist

    try {
    this.emit('update', `[Client] => events register starting register for path ${filePath}`)
   ...
   registerDefaultCommands(client, ignoredcommands = {}){
    ...
    try {
        this.emit('update', `[Client] => Command register starting To register Default commands`)
        register.registerCommands(filePath, this.Commands, client, ignoredcommands)
    } catch(err){
        this.emit('registerFail', err)
        this.emit('update', `[Client] => Command register failed to register Default commands in dir ${filePath}`)
    }
    return true;
   }
   ...
   registerDefaultEvents(client, ignoredEvents = {}){
    ...
    try {
        this.emit('update', `[Client] => events register starting register default events`)
